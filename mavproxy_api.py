        ...
        for m in sorted(self.msgs.keys()):
            if pattern is not None:
                if not fnmatch.fnmatch(str(m).upper(), pattern.upper()):
                    continue
                if getattr(self.msgs[m], '_instance_field', None) is not None and m.find('[') == -1 and pattern.find(
                        '*') != -1:
    ...
    def __init__(self):
        ...
        self.settings = MPSettings(
            [MPSetting('link', int, 1, 'Primary Link', tab='Link', range=(0, 100), increment=1),
             ...
             MPSetting('timeout', int, 5, 'Number of seconds with no packets for a link to considered down',
...
def get_mav_param(param, default=None):
    ...
    vals = mpstate.mav_param.get(param, default)
    print(vals)
    return vals
...
def param_set(name, value, retries=3):
    ...
    vals = mpstate.mav_param.mavset(mpstate.master(), name, value, retries=retries)
    print(vals)
    return
...
def generate_kwargs(args):
    ...
    if (len(module_components) == 2 and module_components[1].endswith("}")):
        ...
        try:
            ...
            print(f'Invalid JSON argument: {module_args} ({repr(e)})')
...
if __name__ == '__main__':
    ...
    print("starting argparser")
    # parser = OptionParser("mavproxy.py [options]")

    parser = argparse.ArgumentParser(
        prog='mavproxy',
        description='starts mav proxy',
        epilog='')

    print("adding options")
    parser.add_argument("--master", dest="master", action='append', metavar="DEVICE[,BAUD]",
                        help="MAVLink master port and optional baud rate", default=[])
    parser.add_argument("--force-connected", dest="force_connected", help="Use master even if initial connection fails",
                        action='store_true', default=False)
    parser.add_argument("--out", dest="output", action='append', metavar="DEVICE[,BAUD]",
                        help="MAVLink output port and optional baud rate", default=[])
    parser.add_argument("--baudrate", dest="baudrate", type=int, help="default serial baud rate", default=57600)
    parser.add_argument("--sitl", dest="sitl", default=None, help="SITL output port")
    parser.add_argument("--streamrate", dest="streamrate", default=4, type=int, help="MAVLink stream rate")
    parser.add_argument("--source-system", dest='SOURCE_SYSTEM', type=int, default=255,
                        help='MAVLink source system for this GCS')
    parser.add_argument("--source-component", dest='SOURCE_COMPONENT', type=int, default=230,
                        help='MAVLink source component for this GCS')
    parser.add_argument("--target-system", dest='TARGET_SYSTEM', type=int, default=0,
                        help='MAVLink target master system')
    parser.add_argument("--target-component", dest='TARGET_COMPONENT', type=int, default=0,
                        help='MAVLink target master component')
    parser.add_argument("--logfile", dest="logfile", help="MAVLink master logfile", default='mav.tlog')
    parser.add_argument("-a", "--append-log", dest="append_log", help="Append to log files", action='store_true',
                        default=False)
    parser.add_argument("--quadcopter", dest="quadcopter", help="use quadcopter controls", action='store_true',
                        default=False)
    parser.add_argument("--setup", dest="setup", help="start in setup mode", action='store_true', default=False)
    parser.add_argument("--nodtr", dest="nodtr", help="disable DTR drop on close", action='store_true', default=False)
    parser.add_argument("--show-errors", dest="show_errors", help="show MAVLink error packets", action='store_true',
                        default=False)
    parser.add_argument("--speech", dest="speech", help="use text to speech", action='store_true', default=False)
    parser.add_argument("--aircraft", dest="aircraft", help="aircraft name", default=None)
    parser.add_argument("--cmd", dest="cmd", help="initial commands", default=None, action='append')
    parser.add_argument("--console", action='store_true', help="use GUI console")
    parser.add_argument("--map", action='store_true', help="load map module")
    ...
    parser.add_argument("--mav20", action='store_true', default=False, help="Use MAVLink protocol 2.0")
    parser.add_argument("--auto-protocol", action='store_true', default=False,
                        help="Auto detect MAVLink protocol version")
    parser.add_argument("--mavversion", choices=['1.0', '2.0'],
                        help="Force MAVLink Version (1.0, 2.0). Otherwise autodetect version")
    parser.add_argument("--nowait", action='store_true', default=False, help="don't wait for HEARTBEAT on startup")
    parser.add_argument("-c", "--continue", dest='continue_mode', action='store_true', default=False,
                        help="continue logs")
    parser.add_argument("--dialect", default="ardupilotmega", help="MAVLink dialect")
    parser.add_argument("--rtscts", action='store_true', help="enable hardware RTS/CTS flow control")
    parser.add_argument("--moddebug", type=int, help="module debug level", default=0)
    parser.add_argument("--mission", dest="mission", help="mission name", default=None)
    parser.add_argument("--daemon", action='store_true', help="run in daemon mode, do not start interactive shell")
    parser.add_argument("--non-interactive", action='store_true', help="do not start interactive shell")
    parser.add_argument("--profile", action='store_true', help="run the Yappi python profiler")
    parser.add_argument("--state-basedir", default=None, help="base directory for logs and aircraft directories")
    ...
    opts = parser.parse_args(args=sys.argv[1:], namespace=None)
